...
export default async function runVersioning(args) {
  ...
  if (versioningExists) {
    ...
    const versionNodes = findVersionNodes(docsConfig.navigation);
    versionNodes.forEach((node) => {
      const label = node.path.join('.');
      console.log(chalk.gray(`  ${label}:`));
      node.owner.versions.forEach(v => {
    ...
    if (!isTopLevelVersionedNavigation(docsConfig.navigation)) {
      console.log(chalk.yellow('\nNested/product-scoped versioning detected.'));
      console.log(chalk.green('Mintlifier can freeze a selected versioned scope.'));
      console.log(chalk.cyan('\nAvailable scoped freeze commands:'));
      findVersionScopes(docsConfig.navigation).forEach((scope) => {
        const alias = scope.aliases.find((value) => !value.includes(':')) || scope.id;
        console.log(chalk.gray(`  npx mintlifier freeze --scope ${alias} --version <version> --next-version <next>`));
      });
      return;
    }
