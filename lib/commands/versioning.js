...
export default async function runVersioning(args) {
  ...
  if (versioningExists) {
    ...
    if (!isTopLevelVersionedNavigation(docsConfig.navigation)) {
      ...
      console.log(chalk.yellow('Mintlifier will not rewrite this structure with the legacy flat-site freezer.'));
      console.log(chalk.gray('Use the project-specific versioning workflow for this repo until the product-scoped freezer is implemented.'));
      return;
    }
