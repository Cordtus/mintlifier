...
export async function setupVersioning(outputDir) {
  ...
  // Ask about latest version visibility
  const latestVisibility = await select({
    message: 'How should the latest (in-development) version be shown?',
    choices: [
      ...
      { value: 'custom', name: 'Use custom label' },
      { value: 'hidden', name: "Don't show in navigation (only stable versions)" }
    ],
    default: 'latest'
  });

  if (latestVisibility === 'custom') {
    versioningConfig.latestLabel = await input({
      message: 'Enter custom label for latest version:',
      default: 'development'
    });
  } else if (latestVisibility === 'hidden') {
    versioningConfig.latestLabel = '';
    versioningConfig.showLatest = false;
  } else {
    versioningConfig.latestLabel = latestVisibility;
  ...
  const versionsData = {
    versions: versioningConfig.showLatest !== false ? [versioningConfig.latestLabel || 'latest'] : [],
    defaultVersion: versioningConfig.showLatest !== false ? versioningConfig.latestLabel || 'latest' : null,
    currentVersion: versioningConfig.initialVersion,
    latestLabel: versioningConfig.latestLabel || 'latest',
  ...
  if (await fs.pathExists(docsJsonPath)) {
    ...
    if (docsConfig.navigation.groups || docsConfig.navigation.tabs) {
      const currentNav = {
        version: versioningConfig.latestLabel || 'latest',
        default: true,
        tabs: docsConfig.navigation.tabs || [
          {
            tab: 'Documentation',
      ...
      docsConfig.navigation = {
        versions: versioningConfig.showLatest !== false ? [currentNav] : []
      };
    } else if (!docsConfig.navigation.versions) {
      // Create new versioned navigation
      docsConfig.navigation.versions = versioningConfig.showLatest !== false ? [
        {
          version: versioningConfig.latestLabel || 'latest',
          default: true,
          tabs: []
    ...
    // Add changelog to navigation if not already there and if showing latest
    if (versioningConfig.showLatest !== false) {
      const latestVersion = docsConfig.navigation.versions.find(v => 
        v.version === (versioningConfig.latestLabel || 'latest')
      );
      if (latestVersion && latestVersion.tabs) {
        // Check if changelog tab exists
        const hasChangelog = latestVersion.tabs.some(tab => 
          tab.tab && tab.tab.toLowerCase().includes('changelog')
        );
        
        if (!hasChangelog) {
          // Add changelog tab
          latestVersion.tabs.push({
            tab: 'Changelog',
  ...
  console.log(chalk.cyan('\nℹ️  To freeze a version and create a new one:'));
  console.log(chalk.gray('   ./scripts/version-manager.sh'));
  
  if (versioningConfig.externalRepo) {
    console.log(chalk.cyan('\nℹ️  To update changelog from external repo:'));
