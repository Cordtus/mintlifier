...
function rewriteLinks(content, plan) {
  return content.replace(/\]\(([^\s)]+)(\s+(?:"[^"]*"|'[^']*'))?\)/g, (match, link, title = '') => {
    ...
    const hasKnownVersion = pathPart.split('/').some((segment) => plan.knownVersions.includes(segment));
    if (!hasKnownVersion) return match;
...
export async function recoverInterruptedFreeze({ transactionPath }) {
  ...
  const docsJsonPath = resolveWithin(root, transaction.docsJsonPath, 'transaction docs config');
  const versionsJsonPath = resolveWithin(root, transaction.versionsJsonPath, 'transaction versions config');
  const cleanupPaths = [
    transaction.stagingDir,
    transaction.docsTemporary,
    transaction.versionsTemporary
  ].filter(Boolean).map((entry) => resolveWithin(root, entry, 'transaction cleanup path'));

  if (transaction.state !== 'committed') {
    for (const target of transaction.targets || []) {
      await fs.remove(resolveWithin(root, target, 'transaction target'));
    }
...
export async function preflightFileCopies(baseDir, fileCopies) {
  ...
  for (const copy of fileCopies) {
    const sourcePath = resolveWithin(baseDir, copy.source, 'freeze source');
    const targetPath = resolveWithin(baseDir, copy.target, 'freeze target');
    if (!await fs.pathExists(sourcePath)) {
      throw new Error(`Source page not found for scoped freeze: ${copy.source}`);
    }
    const sourceStats = await fs.lstat(sourcePath);
    if (sourceStats.isSymbolicLink()) {
      throw new Error(`Source page must not be a symbolic link: ${copy.source}`);
    }
...
export async function applyScopedFreezePlan({
  ...
  const metadataPath = resolveWithin(docsDir, metadataRelativePath, 'freeze metadata');
  ...
  const targets = [
    ...plan.fileCopies.map((copy) => resolveWithin(docsDir, copy.target, 'freeze target')),
    metadataPath
  ];
  const transaction = {
    state: 'prepared',
  ...
  try {
    ...
    for (const copy of plan.fileCopies) {
      const sourcePath = resolveWithin(docsDir, copy.source, 'freeze source');
      const stagedPath = resolveWithin(stagingDir, copy.target, 'freeze target');
      await fs.ensureDir(path.dirname(stagedPath));
      const content = await fs.readFile(sourcePath, 'utf8');
      await fs.writeFile(stagedPath, rewriteLinks(content, plan));
    }
    await fs.outputJson(resolveWithin(stagingDir, metadataRelativePath, 'freeze metadata'), metadata, { spaces: 2 });
    ...
    for (const relativePath of [
      ...
      const stagedPath = resolveWithin(stagingDir, relativePath, 'freeze target');
      const targetPath = resolveWithin(docsDir, relativePath, 'freeze target');
