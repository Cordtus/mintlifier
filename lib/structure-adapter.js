...
export async function analyzeDocumentationFiles(projectPath, structure) {
  ...
  try {
    ...
    // Load config
    const configPath = path.join(projectPath, structure.basePath || '.', structure.configFile);
    if (await fs.pathExists(configPath)) {
      analysis.config = await fs.readJson(configPath);
    }
...
export async function verifyVersioningCompatibility(projectPath, structure, analysis) {
  ...
  if (structure.configFile.includes('mint.json')) {
    issues.push('Using legacy mint.json format');
    recommendations.push('Migrate to docs.json format using Mintlify migration tool');
  }
  ...
  return {
    ...
    summary: {
      ...
      configFormat: structure.configFile.includes('mint.json') ? 'mint.json' : 'docs.json'
    }
  };
}
...
export async function adaptProjectForVersioning(projectPath, structure, analysis) {
  ...
  try {
    ...
    const configPath = path.join(projectPath, structure.basePath || '.', structure.configFile);
    const mintPath = path.join(projectPath, structure.basePath || '.', 'mint.json');
    
    if (await fs.pathExists(mintPath) && !await fs.pathExists(configPath)) {
      const mintConfig = await fs.readJson(mintPath);
      
      // Update schema reference
      mintConfig.$schema = 'https://mintlify.com/docs.json';
      
      await fs.writeJson(configPath.replace('mint.json', 'docs.json'), mintConfig, { spaces: 2 });
    ...
    return {
      ...
      finalStructure: {
        ...
        versionsPath,
        scriptsPath,
        configPath: configPath.replace('mint.json', 'docs.json')
      }
    };
