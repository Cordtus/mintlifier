...
export function normalizeDocsConfig(inputConfig = {}) {
  ...
  if (config.api?.openapi) {
    const normalizeOpenApiPath = (reference) =>
      typeof reference === 'string' && !/^https?:\/\//.test(reference)
        ? reference.replace(/^\/+/, '')
        : reference;
    config.api.openapi = Array.isArray(config.api.openapi)
      ? config.api.openapi.map(normalizeOpenApiPath)
      : normalizeOpenApiPath(config.api.openapi);
  }
