...
export async function applyVersionSetupPlan(plan) {
  const resolvedMoves = await Promise.all(plan.fileMoves.map(async (move) => ({
    source: await resolveWithinWithoutSymlinks(plan.layout.configDir, move.source, 'version source'),
    target: await resolveWithinWithoutSymlinks(plan.layout.configDir, move.target, 'version target'),
