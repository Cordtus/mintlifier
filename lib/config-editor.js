...
async function editNavigation(config) {
  ...
  if (Array.isArray(config.navigation)) {
    currentType = 'simple'; // Traditional array format
  ...
  const navType = await select({
    ...
    choices: [
      { name: 'Simple Array (legacy)', value: 'simple' },
  ...
  switch (navType) {
    case 'simple':
      // Legacy array format
      console.log(chalk.gray('Note: Array format is legacy. Consider upgrading to versioned navigation.'));
      const simplePages = await input({
        message: 'Enter pages (comma-separated):',
        default: Array.isArray(config.navigation) ? config.navigation.join(', ') : 'index, getting-started, api-reference'
      });
      config.navigation = simplePages.split(',').map(p => p.trim());
      break;
    ...
    case 'versions':
      ...
      if (versionOp === 'convert') {
        // Convert existing navigation to versioned format
        ...
        let existingNav;
        if (Array.isArray(config.navigation)) {
          // Convert array to grouped structure
          existingNav = {
            tabs: [{
              tab: 'Documentation',
              groups: [{
          ...
          existingNav = { tabs: config.navigation.tabs };
        }
